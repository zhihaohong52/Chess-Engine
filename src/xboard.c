...
/**
 * @brief Checks if there is threefold repetition
 *
 * @param pos Pointer to the board
 * @return int 2 if there is threefold repetition, 1 or 0 otherwise
 */
...
/**
 * @brief Checks if it is possible to claim a draw
 *
 * @param pos Pointer to the board
...
int DrawMaterial(const S_BOARD *pos) {
    ...
    if (pos->pceNum[wN]!=0 && pos->pceNum[wB]!=0) {return FALSE;}
    if (pos->pceNum[bN]!=0 && pos->pceNum[bB]!=0) {return FALSE;}
 ...
 *
 * @param pos Pointer to the board
 * @return int TRUE if the game is a draw, FALSE otherwise
 */
...
/**
 * @brief Main loop to handle console mode
 *
 * @param pos Pointer to the board
 * @param info Pointer to the search info
 */
